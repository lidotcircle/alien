...
void alien_callback_call(ffi_cif *cif, void *resp, void **args, void *data) {
    ...
    lua_getglobal(L, "callback_mutex");
    std::recursive_mutex* tmx = static_cast<std::recursive_mutex*>(lua_touserdata(L, -1));
    if (tmx == nullptr) {
        tmx = new std::recursive_mutex();
        lua_pushlightuserdata(L, tmx);
        lua_setglobal(L, "callback_mutex");
    }
    lua_pop(L, 1);
    std::lock_guard<std::recursive_mutex> guard(*tmx);
    ...
    switch(ac->ret_type) {
        ...
        case AT_string: *((char**)resp) = lua_isuserdata(L, -1) ? (char*)alien_touserdata(L, -1) : (char *)lua_tostring(L, -1); break;
...
int alien_callback_new(lua_State *L) {
    ...
    if(status == FFI_OK)
        status = ffi_prep_closure_loc((ffi_closure*)ac->fn, &(ac->cif), &alien_callback_call, ac, ac->ffi_codeloc);
    if(status != FFI_OK) {
